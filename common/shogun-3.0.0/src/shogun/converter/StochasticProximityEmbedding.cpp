...
{
	SG_ADD4(&m_k, "m_k", "Number of neighbors", MS_NOT_AVAILABLE);
	SG_ADD4((machine_int_t*) &m_strategy, "m_strategy", "SPE strategy",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_tolerance, "m_tolerance", "Regularization parameter",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_max_iteration, "max_iteration", "maximum number of iterations",
			MS_NOT_AVAILABLE);
}
