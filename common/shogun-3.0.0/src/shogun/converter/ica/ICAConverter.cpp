...
void CICAConverter::init()
{
	...
	SG_ADD4(&m_mixing_matrix, "mixing_matrix", "the mixing matrix", MS_NOT_AVAILABLE);
	SG_ADD4(&max_iter, "max_iter", "maximum number of iterations", MS_NOT_AVAILABLE);
	SG_ADD4(&tol, "tol", "the convergence tolerance", MS_NOT_AVAILABLE);
}
