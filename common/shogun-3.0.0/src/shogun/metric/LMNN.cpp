...
void CLMNN::init()
{
	SG_ADD4(&m_linear_transform, "linear_transform",
			"Linear transform in matrix form", MS_NOT_AVAILABLE)
	SG_ADD4((CSGObject**) &m_features, "features", "Training features",
			MS_NOT_AVAILABLE)
	SG_ADD4((CSGObject**) &m_labels, "labels", "Training labels",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_k, "k", "Number of target neighbours per example",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_regularization, "regularization", "Regularization",
			MS_AVAILABLE)
	SG_ADD4(&m_stepsize, "stepsize", "Step size in gradient descent",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_stepsize_threshold, "stepsize_threshold", "Step size threshold",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_maxiter, "maxiter", "Maximum number of iterations",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_correction, "correction",
			"Iterations between exact impostors search", MS_NOT_AVAILABLE)
	SG_ADD4(&m_obj_threshold, "obj_threshold", "Objective threshold",
			MS_NOT_AVAILABLE)
	SG_ADD4(&m_diagonal, "m_diagonal", "Diagonal transformation", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**) &m_statistics, "statistics", "Training statistics",
			MS_NOT_AVAILABLE);
...
{
	SG_ADD4(&obj, "obj", "Objective at each iteration", MS_NOT_AVAILABLE);
	SG_ADD4(&stepsize, "stepsize", "Step size at each iteration", MS_NOT_AVAILABLE);
	SG_ADD4(&num_impostors, "num_impostors", "Number of impostors at each iteration",
			MS_NOT_AVAILABLE);
}
