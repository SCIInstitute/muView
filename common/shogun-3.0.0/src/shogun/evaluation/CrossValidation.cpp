...
void CCrossValidation::init()
{
	...
	SG_ADD4(&m_num_runs, "num_runs", "Number of repetitions",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_conf_int_alpha, "conf_int_alpha", "alpha-value "
			"of confidence interval", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_xval_outputs, "m_xval_outputs", "List of output "
...
float64_t CCrossValidation::evaluate_one_run()
{
	...
	if (m_machine->is_data_locked())
	{
		...
		for (index_t i=0; i <num_subsets; ++i)
		{
			...
			while (current)
			{
				current->update_train_indices(inverse_subset_indices, "shogun\t");
				current->update_trained_machine(m_machine, "shogun\t");
				SG_UNREF(current);
				current=(CCrossValidationOutput*)
						m_xval_outputs->get_next_element();
			}
			...
			while (current)
			{
				current->update_test_indices(subset_indices, "shogun\t");
				current->update_test_result(result_labels, "shogun\t");
				current->update_test_true_result(m_labels, "shogun\t");
				current->post_update_results();
				current->update_evaluation_result(results[i], "shogun\t");
				SG_UNREF(current);
				current=(CCrossValidationOutput*)
						m_xval_outputs->get_next_element();
			}
