...
class CCrossValidationResult : public CEvaluationResult
{
	public:
		...
		{
			SG_ADD4(&mean, "mean", "Mean of results", MS_NOT_AVAILABLE);
			SG_ADD4(&has_conf_int, "has_conf_int", "Has confidence intervals?",
					MS_NOT_AVAILABLE);
			SG_ADD4(&conf_int_low, "conf_int_low", "Lower confidence bound",
					MS_NOT_AVAILABLE);
			SG_ADD4(&conf_int_up, "conf_int_up", "Upper confidence bound",
						MS_NOT_AVAILABLE);

			SG_ADD4(&conf_int_alpha, "conf_int_alpha",
