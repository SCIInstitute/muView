		  ...
		  v_array<v_array<ds_node<P> > >& stack)
{
  if (point_set.index == 0)
	...
	return n;
      ...
      {
	v_array<ds_node<P> > _far = pop(stack);
	split(point_set,_far,max_scale); //O(|point_set|)
	...
	if (point_set.index == 0)
	  ...
	  while (point_set.index != 0) { //O(|point_set| * num_children)
	    ...
	    dist_split(_far,new_point_set,new_point,max_scale); //O(|far|)
