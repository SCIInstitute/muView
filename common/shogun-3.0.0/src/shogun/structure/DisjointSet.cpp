...
CDisjointSet::CDisjointSet()
	: CSGObject()
{
	SG_UNSTABLE("CDisjointSet::CDisjointSet()", "shogun\n");
...
{
	SG_ADD4(&m_num_elements, "num_elements", "Number of elements", MS_NOT_AVAILABLE);
	SG_ADD4(&m_parent, "parent", "Parent pointers", MS_NOT_AVAILABLE);
	SG_ADD4(&m_rank, "rank", "Rank of each element", MS_NOT_AVAILABLE);
	SG_ADD4(&m_is_connected, "is_connected", "Whether disjoint sets have been linked", MS_NOT_AVAILABLE);
