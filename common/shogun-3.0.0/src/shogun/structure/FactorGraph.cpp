...
CFactorGraph::CFactorGraph()
	: CSGObject()
{
	SG_UNSTABLE("CFactorGraph::CFactorGraph()", "shogun\n");
...
{
	SG_ADD4(&m_cardinalities, "cardinalities", "Cardinalities", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_factors, "factors", "Factors", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_datasources, "datasources", "Factor data sources", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_dset, "dset", "Disjoint set", MS_NOT_AVAILABLE);
	SG_ADD4(&m_has_cycle, "has_cycle", "Whether has circle in graph", MS_NOT_AVAILABLE);
	SG_ADD4(&m_num_edges, "num_edges", "Number of edges", MS_NOT_AVAILABLE);
}
