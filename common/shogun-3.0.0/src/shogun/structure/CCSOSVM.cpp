...
void CCCSOSVM::init()
{
	...
	SG_ADD4(&m_C, "m_C", "C", MS_NOT_AVAILABLE);
	SG_ADD4(&m_eps, "m_eps", "Epsilon", MS_NOT_AVAILABLE);
	SG_ADD4(&m_alpha_thrld, "m_alpha_thrld", "Alpha threshold", MS_NOT_AVAILABLE);
	SG_ADD4(&m_cleanup_check, "m_cleanup_check", "Cleanup after given number of iterations", MS_NOT_AVAILABLE);
	SG_ADD4(&m_idle_iter, "m_idle_iter", "Maximum number of idle iteration", MS_NOT_AVAILABLE);
	SG_ADD4(&m_max_iter, "m_max_iter", "Maximum number of iterations", MS_NOT_AVAILABLE);
	SG_ADD4(&m_max_rho, "m_max_rho", "Max rho", MS_NOT_AVAILABLE);
	SG_ADD4(&m_primal_obj, "m_primal_obj", "Primal objective value", MS_NOT_AVAILABLE);
	SG_ADD4((machine_int_t*) &m_qp_type, "m_qp_type", "QP Solver Type", MS_NOT_AVAILABLE);
}
