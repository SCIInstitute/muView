...
{
	SG_UNSTABLE("CFactorType::CFactorType()", "shogun\n");
...
{
	SG_ADD4(&m_type_id, "type_id", "Factor type name", MS_NOT_AVAILABLE);
	SG_ADD4(&m_cards, "cards", "Cardinalities", MS_NOT_AVAILABLE);
	SG_ADD4(&m_cumprod_cards, "cumprod_cards", "Cumulative product of cardinalities", MS_NOT_AVAILABLE);
	SG_ADD4(&m_num_assignments, "num_assignments", "Number of variable configurations", MS_NOT_AVAILABLE);
	SG_ADD4(&m_w, "w", "Factor parameters", MS_NOT_AVAILABLE);
	SG_ADD4(&m_data_size, "data_size", "Size of data vector", MS_NOT_AVAILABLE);
