...
{
	SG_UNSTABLE("CFactor::CFactor()", "shogun\n");
	init();
}
...
void CFactor::init()
{
	SG_ADD4((CSGObject**)&m_factor_type, "type_name", "Factor type name", MS_NOT_AVAILABLE);
	SG_ADD4(&m_var_index, "var_index", "Factor variable index", MS_NOT_AVAILABLE);
	SG_ADD4(&m_energies, "energies", "Factor energies", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_data_source, "data_source", "Factor data source", MS_NOT_AVAILABLE);
	SG_ADD4(&m_data, "data", "Factor data", MS_NOT_AVAILABLE);
	SG_ADD4(&m_data_sparse, "data_sparse", "Sparse factor data", MS_NOT_AVAILABLE);
	SG_ADD4(&m_is_data_dep, "is_data_dep", "Factor is data dependent or not", MS_NOT_AVAILABLE);
...
{
	SG_ADD4(&m_dense, "dense", "Shared data", MS_NOT_AVAILABLE);
	SG_ADD4(&m_sparse, "sparse", "Shared sparse data", MS_NOT_AVAILABLE);
}
