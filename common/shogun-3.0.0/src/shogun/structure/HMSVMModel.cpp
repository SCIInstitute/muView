...
{
	SG_ADD4((CSGObject**) &m_state_model, "m_state_model", "The state model", MS_NOT_AVAILABLE);
	SG_ADD4(&m_transmission_weights, "m_transmission_weights",
			"Transmission weights used in Viterbi", MS_NOT_AVAILABLE);
	SG_ADD4(&m_emission_weights, "m_emission_weights",
			"Emission weights used in Viterbi", MS_NOT_AVAILABLE);
	SG_ADD4(&m_num_plif_nodes, "m_num_plif_nodes", "The number of points per PLiF",
			MS_NOT_AVAILABLE); // FIXME It would actually make sense to do MS for this parameter
	SG_ADD4(&m_use_plifs, "m_use_plifs", "Whether to use plifs", MS_NOT_AVAILABLE);
