...
{
#ifdef HAVE_LAPACK
	SG_ADD4(&m_opt_max_iterations, "opt_max_iterations", "Maximum number of "
			"iterations for qp solver", MS_NOT_AVAILABLE);
	SG_ADD4(&m_opt_epsilon, "opt_epsilon", "Stopping criterion for qp solver",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_opt_low_cut, "opt_low_cut", "Low cut value for optimization "
