...
{
	SG_ADD4(&m_max_num_iter, "m_max_num_iter", "max number of iterations in alternating optimization", MS_NOT_AVAILABLE);
	SG_ADD4(&m_svm_C, "m_svm_C", "C for svm", MS_AVAILABLE);
	SG_ADD4(&m_A, "m_A", "parameter A", MS_AVAILABLE);
	SG_ADD4(&m_B, "m_B", "parameter B", MS_AVAILABLE);
	SG_ADD4(&m_svm_epsilon, "m_svm_epsilon", "epsilon for svm", MS_AVAILABLE);
}
