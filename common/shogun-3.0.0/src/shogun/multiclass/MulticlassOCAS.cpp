...
void CMulticlassOCAS::register_parameters()
{
	...
	SG_ADD4(&m_epsilon, "m_epsilon", "solver relative tolerance", MS_NOT_AVAILABLE);
	SG_ADD4(&m_max_iter, "m_max_iter", "max number of iterations", MS_NOT_AVAILABLE);
	SG_ADD4(&m_method, "m_method", "used solver method", MS_NOT_AVAILABLE);
	SG_ADD4(&m_buf_size, "m_buf_size", "buffer size", MS_NOT_AVAILABLE);
}
