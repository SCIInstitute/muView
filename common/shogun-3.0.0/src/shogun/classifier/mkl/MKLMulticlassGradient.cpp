...
void MKLMulticlassGradient::genbetas( ::std::vector<float64_t> & weights ,const ::std::vector<float64_t> & gammas)
{
	...
	for(int32_t i=0; i< numkernels-1 ;++i)
	{
		...
		{
			weights[k]*=cos( MIN(MAX(0.0,gammas[i]),pi4) );
		}
		weights[i+1]=sin( MIN(MAX(0.0,gammas[i]),pi4) );
	}
...
void MKLMulticlassGradient::gengammagradient( ::std::vector<float64_t> & gammagradient ,const ::std::vector<float64_t> & gammas,const int32_t dim)
{
	...
	for(int32_t i=0; i< numkernels-1 ;++i)
	{
		if(i!=dim)
		{
			for(int32_t k=0; k< MIN(i+1,dim+2) ;++k)
			{
				gammagradient[k]*=pow( cos( MIN(MAX(0.0,gammas[i]),pi4) ), 2.0/pnorm) ;
			}
			if(i<dim)
				gammagradient[i+1]=pow( sin( MIN(MAX(0.0,gammas[i]),pi4) ),2.0/pnorm);
		}
		else if(i==dim)
		{
			...
			for(int32_t k=0; k< i+1 ;++k)
			{
				gammagradient[k]*= pow( cos( MIN(MAX(0.0,gammas[i]),pi4) ), 2.0/pnorm-1.0)*(-1)*sin( MIN(MAX(0.0,gammas[i]),pi4) );
			}
			gammagradient[i+1]=pow( sin( MIN(MAX(0.0,gammas[i]),pi4) ),2.0/pnorm-1)*cos( MIN(MAX(0.0,gammas[i]),pi4) );
		}
	}
}
...
void MKLMulticlassGradient::linesearch(std::vector<float64_t> & finalbeta,const std::vector<float64_t> & oldweights)
{
			...
			for(int32_t k=numkernels-2; k>= 1 ;--k) // k==0 not necessary
			{
				if(cos(curgamma[i-1])>0)
				{
                  ...
                  SG_SINFO("linesearch(...): at k %d tmpbeta[k]>1 %f\n",k, tmpbeta[k])
					...
					tmpbeta[k]=MIN(1.0,MAX(0.0, tmpbeta[k]));
				}
			}
		}
	}
	...
	while(!finished)
	{
		...
		for(int32_t i=0; i< numkernels-1 ;++i)
		{
			maxgrad=MAX(maxgrad,fabs(curgrad[i]) );
			if(curgrad[i]<0)
			{
		...
		{
			maxalpha=MIN(maxalpha,maxalphas[i]);
		}
		...
		// curgamma + al*curgrad ,aximizes al in [0, maxal]
			...
			for(int32_t i=1; i< (int32_t)sumsofalphas.size() ;++i)
			{
				curobj=MIN(curobj,objectives(tmpbeta, i));
			}
         ...
         float64_t tmpobj=MAX(lobj,robj);
			do
			{

				tmpobj=MAX(lobj,robj);
