...
#ifdef WIN32
	prediction_fd = fopen(file_name, "w");
#else
...
#ifdef WIN32
void CVowpalWabbit::output_prediction(FILE * f, float32_t res, float32_t weight, v_array<char> tag)
#else
...
#endif
{
	if (f >= 0)
	{
		...
		ssize_t t;
#ifdef WIN32
		t = fwrite(temp,sizeof(char),num, f);
#else
		t = write(f, temp, num);
#endif
		if (t != num)
...
#ifdef WIN32
				t = fwrite(tag.begin, sizeof(char), tag.index(), f);
#else
