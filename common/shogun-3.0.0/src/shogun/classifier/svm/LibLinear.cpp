...
void CLibLinear::init()
{
	...
	SG_ADD4(&C1, "C1", "C Cost constant 1.", MS_AVAILABLE);
	SG_ADD4(&C2, "C2", "C Cost constant 2.", MS_AVAILABLE);
	SG_ADD4(&use_bias, "use_bias", "Indicates if bias is used.",
			MS_NOT_AVAILABLE);
	SG_ADD4(&epsilon, "epsilon", "Convergence precision.", MS_NOT_AVAILABLE);
	SG_ADD4(&max_iterations, "max_iterations", "Max number of iterations.",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_linear_term, "linear_term", "Linear Term", MS_NOT_AVAILABLE);
	SG_ADD4((machine_int_t*) &liblinear_solver_type, "liblinear_solver_type",
