...
{
	SG_ADD4(&C1, "C1", "", MS_AVAILABLE);
	SG_ADD4(&C2, "C2", "", MS_AVAILABLE);
	SG_ADD4(&svm_loaded, "svm_loaded", "SVM is loaded.", MS_NOT_AVAILABLE);
	SG_ADD4(&epsilon, "epsilon", "", MS_AVAILABLE);
	SG_ADD4(&tube_epsilon, "tube_epsilon",
			"Tube epsilon for support vector regression.", MS_AVAILABLE);
	SG_ADD4(&nu, "nu", "", MS_AVAILABLE);
	SG_ADD4(&objective, "objective", "", MS_NOT_AVAILABLE);
	SG_ADD4(&qpsize, "qpsize", "", MS_NOT_AVAILABLE);
	SG_ADD4(&use_shrinking, "use_shrinking", "Shrinking shall be used.",
			MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**) &mkl, "mkl", "MKL object that svm optimizers need.",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_linear_term, "linear_term", "Linear term in qp.",
			MS_NOT_AVAILABLE);
