...
{
	SG_ADD4(&m_initialized, "initialized", "The prerpocessor is initialized",  MS_NOT_AVAILABLE);
	SG_ADD4(&m_divide_by_std, "divide_by_std", "Divide by standard deviation", MS_AVAILABLE);
	SG_ADD4(&m_num_idx, "num_idx", "Number of elements in idx_vec", MS_NOT_AVAILABLE);
	SG_ADD4(&m_std, "std_vec", "Standard dev vector", MS_NOT_AVAILABLE);
	SG_ADD4(&m_mean, "mean_vec", "Mean vector", MS_NOT_AVAILABLE);
	SG_ADD4(&m_idx, "idx_vec", "Index vector", MS_NOT_AVAILABLE);
}
