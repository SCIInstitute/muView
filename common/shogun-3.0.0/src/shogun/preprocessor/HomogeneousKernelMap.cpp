...
{
	/* register variables */
	SG_ADD4((machine_int_t*) &m_kernel, "kernel", "Kernel type to use.",MS_AVAILABLE);
	SG_ADD4((machine_int_t*) &m_window, "window", "Window type to use.",MS_AVAILABLE);
	SG_ADD4(&m_gamma, "gamma", "Homogeneity order.",MS_AVAILABLE);
	SG_ADD4(&m_period, "period", "Approximation order",MS_NOT_AVAILABLE);
	SG_ADD4(&m_numSubdivisions, "num_subdivisions", "The number of sublevels",MS_NOT_AVAILABLE);
	SG_ADD4(&m_subdivision, "subdivision", "subdivision.",MS_NOT_AVAILABLE);
	SG_ADD4(&m_order, "order", "The order",MS_AVAILABLE);
	SG_ADD4(&m_minExponent, "min_exponent", "Minimum exponent",MS_NOT_AVAILABLE);
	SG_ADD4(&m_maxExponent, "max_exponent", "Maximum exponent",MS_NOT_AVAILABLE);
	SG_ADD4(&m_table, "table", "Lookup-table",MS_NOT_AVAILABLE);
}
