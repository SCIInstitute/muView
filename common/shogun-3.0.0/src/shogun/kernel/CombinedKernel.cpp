...
bool CCombinedKernel::init(CFeatures* l, CFeatures* r)
{
	...
	for (index_t k_idx=0; k_idx<get_num_kernels() && result; k_idx++)
	{
		...
		if (k->get_kernel_type() != K_CUSTOM)
		{
			...
			SG_DEBUG("Initializing 0x%p - \"%s\"shogun\n", this, k->get_name())
			result=k->init(lf,rf);
			SG_UNREF(lf);
			SG_UNREF(rf);
...
void CCombinedKernel::init()
{
	...
	SG_ADD4((CSGObject**) &kernel_array, "kernel_array", "Array of kernels.",
	    MS_AVAILABLE);
	...
	SG_ADD4(&append_subkernel_weights, "append_subkernel_weights",
	    "If subkernel weights are appended.", MS_AVAILABLE);
	SG_ADD4(&initialized, "initialized", "Whether kernel is ready to be used.",
	    MS_NOT_AVAILABLE);
}
