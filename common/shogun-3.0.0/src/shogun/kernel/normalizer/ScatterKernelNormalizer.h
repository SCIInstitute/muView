...
private:
	void init()
	{
		...
		SG_ADD4(&m_testing_class, "m_testing_class",
				"Testing Class.", MS_NOT_AVAILABLE);
		SG_ADD4(&m_const_diag, "m_const_diag",
				"Factor to multiply to diagonal elements.", MS_AVAILABLE);
		SG_ADD4(&m_const_offdiag, "m_const_offdiag",
				"Factor to multiply to off-diagonal elements.", MS_AVAILABLE);

		SG_ADD4((CSGObject**) &m_labels, "m_labels", "Labels", MS_NOT_AVAILABLE);
		SG_ADD4((CSGObject**) &m_normalizer, "m_normalizer", "Kernel normalizer.",
		    MS_AVAILABLE);
	}
