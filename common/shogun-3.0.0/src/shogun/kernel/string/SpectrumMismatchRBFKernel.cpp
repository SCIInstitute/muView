...
{
	SG_ADD4(&degree, "degree", "degree of the kernel", MS_AVAILABLE);
	SG_ADD4(&AA_matrix, "AA_matrix", "128*128 scalar product matrix",
			MS_NOT_AVAILABLE);
	SG_ADD4(&width, "width", "width of Gaussian", MS_AVAILABLE);
	SG_ADD4(&target_letter_0, "target_letter_0", "target letter 0",
			MS_NOT_AVAILABLE);
	SG_ADD4(&initialized, "initialized", "the mark of initialization status",
			MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject** )&kernel_matrix, "kernel_matrix",
...
{
	SG_ADD4((CSGObject** )&alphabet, "alphabet", "the alphabet used by kernel",
			MS_NOT_AVAILABLE);
}
