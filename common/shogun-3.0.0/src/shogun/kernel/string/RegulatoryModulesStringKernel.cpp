...
void CRegulatoryModulesStringKernel::init()
{
	...
	SG_ADD4(&width, "width", "the width of Gaussian kernel part", MS_AVAILABLE);
	SG_ADD4(&degree, "degree", "the degree of weighted degree kernel part",
	    MS_AVAILABLE);
	SG_ADD4(&shift, "shift",
	    "the shift of weighted degree with shifts kernel part", MS_AVAILABLE);
	SG_ADD4(&window, "window", "the size of window around motifs", MS_AVAILABLE);
	SG_ADD4((CSGObject**)&motif_positions_lhs, "motif_positions_lhs",
			"the matrix of motif positions from sequences left-hand side", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&motif_positions_rhs, "motif_positions_rhs",
			"the matrix of motif positions from sequences right-hand side", MS_NOT_AVAILABLE);
	SG_ADD4(&position_weights, "position_weights", "scaling weights in window", MS_NOT_AVAILABLE);
	SG_ADD4(&weights, "weights", "weights of WD kernel", MS_NOT_AVAILABLE);
}
