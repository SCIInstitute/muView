...
void CLocalAlignmentStringKernel::init()
{
	...
	SG_ADD4(&initialized, "initialized", "If kernel is initalized.",
		MS_NOT_AVAILABLE);
	SG_ADD4(&m_opening, "opening", "Opening gap opening penalty.", MS_AVAILABLE);
	SG_ADD4(&m_extension, "extension", "Extension gap extension penalty.",
		MS_AVAILABLE);
}
