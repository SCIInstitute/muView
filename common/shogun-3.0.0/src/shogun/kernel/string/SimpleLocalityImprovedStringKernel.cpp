...
void CSimpleLocalityImprovedStringKernel::init()
{
	...
	SG_ADD4(&length, "length", "Window Length.", MS_AVAILABLE);
	SG_ADD4(&inner_degree, "inner_degree", "Inner degree.", MS_AVAILABLE);
	SG_ADD4(&outer_degree, "outer_degree", "Outer degree.", MS_AVAILABLE);
	SG_ADD4(&pyramid_weights,"pyramid_weights", "Pyramid weights.", MS_AVAILABLE);
}
