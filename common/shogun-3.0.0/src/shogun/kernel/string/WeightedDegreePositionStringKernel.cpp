...
void CWeightedDegreePositionStringKernel::init()
{
	...
	m_parameters->add_vector(&shift, &shift_len,
			"shift",
			"Shift Vector.");
	SG_ADD4(&max_shift, "max_shift", "Maximal shift.", MS_AVAILABLE);
	SG_ADD4(&mkl_stepsize, "mkl_stepsize", "MKL step size.", MS_AVAILABLE);
	SG_ADD4(&degree, "degree", "Order of WD kernel.", MS_AVAILABLE);
	SG_ADD4(&max_mismatch, "max_mismatch",
			"Number of allowed mismatches.", MS_AVAILABLE);
	SG_ADD4(&block_computation, "block_computation",
			"If block computation shall be used.", MS_NOT_AVAILABLE);
	SG_ADD4((machine_int_t*) &type, "type",
			"WeightedDegree kernel type.", MS_AVAILABLE);
	SG_ADD4(&which_degree, "which_degree",
			"The selected degree. All degrees are used by default (for value -1).",
			MS_AVAILABLE);
	SG_ADD4((CSGObject**) &alphabet, "alphabet",
