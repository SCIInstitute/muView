	...
	public:
			...
			SGVector<T> tri_kernel_matrix)
		{
			...
			SG_DEBUG("using custom kernel of size %dx%d\n", cols,cols)
				...
				kmatrix = SGMatrix<float32_t>(SG_MALLOC(float32_t, len), (index_t)cols, (index_t)cols);
			upper_diagonal=true;

			for (int64_t i=0; i<len; i++)
				kmatrix.matrix[i]=(float32_t)tri_kernel_matrix.vector[i];

			dummy_init((int32_t)cols, (int32_t)cols);
			return true;
		}
			...
			SGMatrix<T> full_kernel_matrix)
		{
			...
			for (int64_t row=0; row<rows; row++)
			{
				for (int64_t col=row; col<cols; col++)
				{
					...
					kmatrix.matrix[idx] = (float32_t)full_kernel_matrix.matrix[col*rows+row];
				}
			}
			...
			SGMatrix<float64_t> full_kernel_matrix)
		{
			...
			for (int64_t i=0; i<int64_t(rows) * cols; i++)
				kmatrix.matrix[i]=(float32_t)full_kernel_matrix.matrix[i];
