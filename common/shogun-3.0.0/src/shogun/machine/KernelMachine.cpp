...
void CKernelMachine::init()
{
	...
	SG_ADD4((CSGObject**) &kernel, "kernel", "", MS_AVAILABLE);
	SG_ADD4((CSGObject**) &m_custom_kernel, "custom_kernel", "Custom kernel for"
			" data lock", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**) &m_kernel_backup, "kernel_backup",
			"Kernel backup for data lock", MS_NOT_AVAILABLE);
	SG_ADD4(&use_batch_computation, "use_batch_computation",
			"Batch computation is enabled.", MS_NOT_AVAILABLE);
	SG_ADD4(&use_linadd, "use_linadd", "Linadd is enabled.", MS_NOT_AVAILABLE);
	SG_ADD4(&use_bias, "use_bias", "Bias shall be used.", MS_NOT_AVAILABLE);
	SG_ADD4(&m_bias, "m_bias", "Bias term.", MS_NOT_AVAILABLE);
	SG_ADD4(&m_alpha, "m_alpha", "Array of coefficients alpha.",
			MS_NOT_AVAILABLE);
	SG_ADD4(&m_svs, "m_svs", "Number of ``support vectors''.", MS_NOT_AVAILABLE);
