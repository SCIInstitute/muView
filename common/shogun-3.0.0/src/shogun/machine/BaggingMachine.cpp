...
{
	SG_ADD4((CSGObject**)&m_features, "features", "Train features for bagging",
		MS_NOT_AVAILABLE);
	SG_ADD4(&m_num_bags, "num_bags", "Number of bags", MS_AVAILABLE);
	SG_ADD4(&m_bag_size, "bag_size", "Number of vectors per bag", MS_AVAILABLE);
	SG_ADD4((CSGObject**)&m_bags, "bags", "Bags array", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_combination_rule, "combination_rule",
		"Combination rule to use for aggregating", MS_AVAILABLE);
	SG_ADD4(&m_all_oob_idx, "all_oob_idx", "Indices of all oob vectors",
			MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**)&m_oob_indices, "oob_indices",
