...
void CProbingSampler::init()
{
	...
	SG_ADD4(&m_coloring_vector, "coloring_vector", "the coloring vector generated"
		" from coloring", MS_NOT_AVAILABLE);

	SG_ADD4(&m_power, "matrix_power", "power of the sparse-matrix for coloring",
		MS_NOT_AVAILABLE);

	SG_ADD4(&m_is_precomputed, "is_precomputed",
		"flag that is true if already precomputed", MS_NOT_AVAILABLE);

	SG_ADD4((CSGObject**)&m_matrix_operator, "matrix_operator",
