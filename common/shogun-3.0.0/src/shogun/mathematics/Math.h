	...
	public:
		...
		*/
		...
		static inline float32_t normal_random(float32_t mean, float32_t std_dev)
		{
			...
			do
			{
				rand_u = (float32_t)CMath::random(-1.0, 1.0);
				rand_v = (float32_t)CMath::random(-1.0, 1.0);
				rand_s = rand_u*rand_u + rand_v*rand_v;
			} while ((rand_s == 0) || (rand_s >= 1));

			// the meat & potatos, and then the mean & standard deviation shifting...
			ret = (float32_t)(rand_u*sqrt(-2.0*log(rand_s) / rand_s));
		...
		{
			return (float32_t)normal_random(0.0, 1.0);
		}
		...
		{
#if defined(isfinite) && !defined(SUNOS) && !defined(_WIN32)
			return isfinite(f);
#elif defined(_WIN32)
			return _finite(f);
#else
