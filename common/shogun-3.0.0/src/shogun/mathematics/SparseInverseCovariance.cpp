...
{
	SG_ADD4(&m_lasso_max_iter,"lasso_max_iter",
	       "maximum iteration of LASSO step",MS_NOT_AVAILABLE);
	SG_ADD4(&m_max_iter,"max_iter","maximum total iteration",
	       MS_NOT_AVAILABLE);
	SG_ADD4(&m_f_gap,"f_gap","f gap",MS_NOT_AVAILABLE);
	SG_ADD4(&m_x_gap,"x_gap","x gap",MS_NOT_AVAILABLE);
	SG_ADD4(&m_xtol,"xtol","xtol",MS_NOT_AVAILABLE);
}
