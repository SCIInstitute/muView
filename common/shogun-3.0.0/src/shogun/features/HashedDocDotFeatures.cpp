	...
	CTokenizer* tzer, bool normalize, int32_t n_grams, int32_t skips)
{
	...
	SG_ADD4(&num_bits, "num_bits", "Number of bits of hash", MS_NOT_AVAILABLE);
	SG_ADD4(&ngrams, "ngrams", "Number of tokens to combine for quadratic feature support",
			MS_NOT_AVAILABLE);
	SG_ADD4(&tokens_to_skip, "tokens_to_skip", "Number of tokens to skip when combining features",
			MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**) &doc_collection, "doc_collection", "Document collection",
			MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject**) &tokenizer, "tokenizer", "Document tokenizer",
			MS_NOT_AVAILABLE);
	SG_ADD4(&should_normalize, "should_normalize", "Normalize or not the dot products",
			MS_NOT_AVAILABLE);
