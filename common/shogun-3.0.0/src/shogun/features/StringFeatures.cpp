...
static struct dirent* new_dirent(struct dirent** dptr, const struct dirent *src) {
	...
	if ((ndirent = (struct dirent *)malloc(sizeof(struct dirent)))) {

		*ndirent = *src;
		//if ((ndirent->d_name = (char*)malloc(ndirent->d_namlen + 1))) {
			strcpy_s(ndirent->d_name, ndirent->d_namlen + 1, src->d_name);
			*dptr = ndirent;
		//}
		//else {
		//	free(ndirent);
	...
	int(*compar)(const void *, const void *))
{
	...
	*namelist = NULL;
	...
	if (filter && !filter(dirent))
		continue;
	else if ((*namelist = (struct dirent **)realloc(*namelist, sizeof(struct dirent*) * (len + 1))) == NULL)
