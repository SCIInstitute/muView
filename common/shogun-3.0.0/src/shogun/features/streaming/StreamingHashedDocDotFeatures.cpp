	...
	int32_t size, CTokenizer* tzer, int32_t bits, bool normalize, int32_t n_grams, int32_t skips)
{
	...
	SG_ADD4(&num_bits, "num_bits", "Number of bits for hash", MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject** ) &tokenizer, "tokenizer", "The tokenizer used on the documents",
		MS_NOT_AVAILABLE);
	SG_ADD4((CSGObject** ) &converter, "converter", "Converter", MS_NOT_AVAILABLE);
